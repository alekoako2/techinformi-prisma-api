...
function setOpArgs(args) {
    ...
    if (args.query) {
        ...
        if (leaderExecutors) {
            opArgs.where.AND.push({OR: [{researchExecutors_contains: leaderExecutors}, {leaderName_contains: leaderExecutors}]});
        }
        if (keywords) {
            opArgs.where.AND.push({translation_some: {key_some: {name_contains: keywords}}});
        }
        if (organizationName) {
        ...
        if (yearResearchProgressStartOrEndYear) {
            let range_to = new Date(yearResearchProgressStartOrEndYear).getFullYear() + 1;
            opArgs.where.AND.push({
                OR: [
                    {AND: [{regDate_gt: yearResearchProgressStartOrEndYear}, {regDate_lt: range_to.toString()}]},
                    {AND: [{startDate_gt: yearResearchProgressStartOrEndYear}, {startDate_lt: range_to.toString()}]},
                    {AND: [{endDate_gt: yearResearchProgressStartOrEndYear}, {endDate_lt: range_to.toString()}]},
                ]
            })
        }
        if (fromYear) {
