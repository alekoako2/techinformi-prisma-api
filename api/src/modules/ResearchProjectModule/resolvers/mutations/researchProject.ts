...
import {
  ...
  OecdUpdateManyInput,
  KeywordCreateManyInput,
  KeywordCreateInput,
  KeywordUpdateManyInput,
} from '@prisma-client'
...
async function setNonTranslatedSchema(schema, input, ctx: Context, id?) {
  ...
  if (input.participatingInstitutionCountryCity) {
    schema.participatingInstitutionCountryCity =
      input.participatingInstitutionCountryCity
  }
    ...
    for (let i = 0; i < input.translation.length; i++) {
      ...
      if (input.translation[i].key) {
        ...
        for (let k = 0; k < input.translation[i].key.length; k++) {
          schema.translation.create[i].key.create[k] = {
            name: input.translation[i].key[k],
          }
        }
      }
...
async function updateResearchProjectSchema(input, id, ctx: Context) {
  ...
  if (input.translation) {
    for (let i = 0; i < input.translation.length; i++) {
      ...
      let translation = await ctx.prisma.researchProject({ id }).translation({
