...
import {
  ...
  NewsTranslationCreateManyInput, NewsTranslationUpdateDataInput,
  NewsTranslationUpdateWithWhereUniqueNestedInput, NewsTranslationWhereUniqueInput,
  NewsUpdateInput,
  LanguageCreateManyInput,
  LanguageCreateOneInput, LanguageUpdateManyInput,
  LanguageWhereUniqueInput,
  OecdCreateManyInput, OecdUpdateManyInput,
  ...
  if (input.translation) {
    ...
    for (let i = 0; i < input.translation.length; i++) {
      ...
      if (input.translation[i].description) {
        schema.translation.create[i].description = input.translation[i].description;
      }
...
async function updateNewsSchema(input, id, ctx: Context) {
  ...
  schema.translation = <NewsTranslationCreateManyInput>{};
  schema.translation.update = <NewsTranslationUpdateWithWhereUniqueNestedInput[]>[];

  if (input.translation) {

    for (let i = 0; i < input.translation.length; i++) {

      schema.translation.update[i] = <NewsTranslationUpdateWithWhereUniqueNestedInput>{};
      ...
      let translation = await ctx.prisma.news({ id }).translation({ where: { language: { code: input.translation[i].language } } });

      if (input.translation[i].title) {
        schema.translation.update[i].data.title = input.translation[i].title;
      }
      if (input.translation[i].description) {
        schema.translation.update[i].data.description = input.translation[i].description;
      }
      if (input.translation[i].content) {
